...
def chunk_tokens_labels(df: pd.DataFrame, max_length: int):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df)):
        ...
        assert (
            len(tokenizer(row["tokens"], is_split_into_words=True)["input_ids"][1:-1])
            == total_subword_length
        )
...
def write_df_to_json(df: pd.DataFrame, path_to_json: str):
    ...
    for i in tqdm(range(len(tokens_list)), total=len(tokens_list)):
        ...
        data_list.append({"index": index, "tokens": tokens, "ner_tags": labels})
...
def prep_data(path_to_file: str, max_length: int, test: bool = False):
    ...
    print(f"{dataset} data length after chunking to max {max_length} tokens: {len(df)}")
...
if __name__ == "__main__":
    convert_parquet_data_to_json(
        ...
        max_length=500,
        output_train_file_name="data_gen_content_train_subword_deberta_v2_500.json",
        output_val_file_name="data_gen_content_val_subword_deberta_v2_500.json",
        output_test_file_name="data_gen_content_test_subword_deberta_v2_500.json",
    )
