...
batch_size = 32
per_device_max_bz = 16
# %%
json_config = {
    "train_file": "data/data_gen_content_train_240.json",
    "validation_file": "data/data_gen_content_val_240.json",
    "test_file": "data/data_gen_content_test_240.json",
    "output_dir": "output_dev/v39",
    "model_name_or_path": "albert/albert-xxlarge-v2",
    "num_train_epochs": 6,
    "per_device_train_batch_size": min(batch_size // n_gpu, per_device_max_bz),
...
json_config['gradient_accumulation_steps'] = batch_size // json_config['per_device_train_batch_size'] // n_gpu
...
assert not data_args.label_all_tokens
...
def compute_metrics(p):
    ...
    f1_all = f1_score(merged_labels, merged_predictions, average=None)
    metrics = {"f1": f1_score(merged_labels, merged_predictions, average="macro")}
    metrics.update({f'f1_{idx}': val for idx, val in enumerate(f1_all)})

    return metrics
    # return {"f1": f1_score(merged_labels, merged_predictions, average="macro")}
...
trainer = Trainer(
    ...
    callbacks=[
        # 默认会添加TensorboardCallback，输出日志由logging_dir指定
